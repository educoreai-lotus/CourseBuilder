...
/**
 * Trusted learner identity for learner-owned data (Phase 2).
 * Only req.user.directoryUserId — never body/query/header IDs.
 */
...
export function assertLearnerMatchesAuthenticatedUser(req, requestedLearnerId) {
  const authenticatedLearnerId = getAuthenticatedLearnerId(req);
  if (
    requestedLearnerId &&
    String(requestedLearnerId).trim() !== '' &&
    String(requestedLearnerId).trim() !== authenticatedLearnerId
  ) {
    const error = new Error('Forbidden: learner mismatch');
    error.status = 403;
    throw error;
  }
  return authenticatedLearnerId;
}
...
export function isPersonalizedLearnerCourse(course) {
  if (!course) {
    return false;
  }
  const learningPathDesignation = course.learning_path_designation || {};
  return (
    course.course_type === 'learner_specific' ||
    Boolean(learningPathDesignation.personalized) ||
    learningPathDesignation.source === 'learner_ai'
  );
}
